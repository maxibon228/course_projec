...
class Stage5AdminTests(unittest.TestCase):
    def setUp(self):
...
class Stage5ServerTests(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.TemporaryDirectory()
        self.csv_path = Path(self.temp_dir.name) / "messages.csv"
    ...
    def test_easter_egg_for_tea_word(self):
        response = self.client.post(
            f"/{srv.STUDENT_ID}",
            data={"text": "Чай"},
            environ_overrides={"REMOTE_ADDR": "127.0.0.1"},
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn("Пасхалка".encode("utf-8"), response.data)
        self.assertIn("Вы на верном этапе".encode("utf-8"), response.data)
        self.assertIn("ссылка на тг бота".encode("utf-8"), response.data)
        self.assertIn("Отправить снова".encode("utf-8"), response.data)
        self.assertIn(f'action="/{srv.STUDENT_ID}"'.encode("utf-8"), response.data)
